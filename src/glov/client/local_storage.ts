...
export function getJSON<T = unknown>(key: string, def?: T): T | undefined {
  let value = get(key);
  if (value === undefined) {
    return def;
  }
...
export function clearAll(key_prefix?: string): void {
...
export function importAll(serialized: string): void {
